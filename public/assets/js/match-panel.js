    ...
    function updateUI(data) {
        ...
        if (list && data.goals) {
            ...
            list.innerHTML = data.goals.map(g =>
                `<div class="panel-goal-item"><span>⚽ ${g.minute ? g.minute + "'" : ''} ${(g.prenume||'')+' '+(g.nume||'')} ${canRemove ? `<button type="button" class="btn btn-sm btn-ghost" data-remove-goal="${g.id}">×</button>` : ''}</span></div>`
            ).join('');
            bindRemoveGoal();
        }
    }
